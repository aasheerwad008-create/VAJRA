"""
VAJRA Voice AI — ECAPA-TDNN Fine-Tuning.

Two-stage transfer learning for the ECAPA-TDNN anti-spoofing classifier:

Stage 1 — Feature Extraction:
    - Load pretrained ECAPA-TDNN backbone (SpeechBrain / fallback TDNN)
    - Freeze backbone, train only the classifier head
    - Learning rate: 1e-4, epochs: 10

Stage 2 — Fine-Tuning:
    - Unfreeze backbone (TDNN fallback) or keep extracting fixed
      embeddings (SpeechBrain)
    - Fine-tune with discriminative learning rates
    - Backbone LR: 1e-5, Head LR: 1e-4, epochs: 5

Usage:
    python -m training.train_ecapa --data-root /data/ASVspoof2024/LA
"""
...
def train_ecapa(
    ...
    """
    Train the ECAPA-TDNN anti-spoofing classifier with two-stage transfer learning.

    Stage 1: Freeze backbone, train classifier head only.
    Stage 2: Unfreeze backbone (fallback TDNN), fine-tune with small LR.

    Returns combined metrics from both stages.
    """
    # ── Build model ───────────────────────────────────────────────────────
    model = ECAPATDNNClassifier(num_classes=2, pretrained=True)
    ...
    trainer = Trainer(model, config, model_name="ecapa_tdnn")
    ...
    # ══════════════════════════════════════════════════════════════════════
    log.info("=" * 70)
    log.info("STAGE 1: ECAPA-TDNN feature extraction — frozen backbone")
    log.info("=" * 70)
    ...
    # ══════════════════════════════════════════════════════════════════════
    log.info("=" * 70)
    log.info("STAGE 2: ECAPA-TDNN fine-tuning — unfreezing backbone")
    log.info("=" * 70)
    ...
    log.info("ECAPA-TDNN training complete: %d total epochs", len(all_metrics))
    return all_metrics
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train ECAPA-TDNN anti-spoofing classifier (pretrained + fine-tuning)"
    )
